...
def get_route(start_point, end_point):
  ...
  for start_station in start_stations:
    for end_station in end_stations:
      metro_systems = get_active_stations() if stations_under_construction else vc_metro
      if len(stations_under_construction) > 0:
        possible_route = dfs(metro_systems, start_station, end_station)
        if possible_route is None:
          return None
      route = bfs(metro_systems, start_station, end_station)
